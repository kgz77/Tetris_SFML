...
void tb::TextButton::click(sf::RenderWindow& window) {
		if (sf::IntRect(17, 30, 100, 30).contains(sf::Mouse::getPosition(window))) {
			text[0].setFillColor(sf::Color::Green);
			
			mainMenu = 1;
			if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
				if (mainMenu == 1) {
					sf::RenderWindow windGame(sf::VideoMode(360, 397), L"Тетрис");
					startGameWindow(windGame);
				}
			}
		}
		else if (sf::IntRect(17, 90, 100, 30).contains(sf::Mouse::getPosition(window))) {
		...
		else if (sf::IntRect(17, 150, 100, 30).contains(sf::Mouse::getPosition(window))) {
